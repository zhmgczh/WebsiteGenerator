    ...
    with open("./component.csv", mode="r", encoding="utf-8") as csv_file:
        ...
        for row in csv_reader:
            if -1 in (id_index, file_index):
                ...
                if "" != row[file_index]:
                    with open(
                        os.path.join("./components/", row[file_index]),
                        mode="r",
                        encoding="utf-8",
                    ) as file:
...
def generate_entries(filepath: str, category_name: str, website_directory: str):
    ...
    for row in rows:
        if -1 in (title_index, content_index, excerpt_index, category_index):
            ...
            cleantext = remove_double_newline_and_truncate(
                BeautifulSoup(row[content_index], "lxml").text
            )
            ...
            article = replace_tag(
                article, "<!--|||||article_category|||||-->", row[category_index]
            )
            article = replace_tag(
                article,
                "<!--|||||article_category_encoded|||||-->",
                quote(row[category_index]),
            )
            article = replace_tag(
                article, "<!--|||||article_title|||||-->", row[title_index]
            )
            article = replace_tag(
                article, "<!--|||||article_post|||||-->", row[content_index]
            )
            title = truncate(row[title_index])
            url = replace_all_components(
                quote(os.path.join("/", row[category_index], title) + "/")
                .replace("//", "/")
                .lower()
            )
            ...
            if (
                url not in content_database
                or content_database[url]["hash_value"] != hash_value.hexdigest()
            ):
                make_directory(title)
                with open(
                    os.path.join("./", title, "index.html"), mode="w", encoding="utf-8"
                ) as file:
                    file.write(article)
            ...
            category_database[row[category_index]].append(
                {
                    "url": url,
                    "title": row[title_index],
                    "description": row[excerpt_index],
        ...
        if "." != category[0]:
            ...
            for article in articles:
                if article.endswith(".html"):
                    with open(
                        os.path.join(original_directory, directory, category, article),
                        mode="r",
                        encoding="utf-8",
                    ) as file:
                        post_content = file.read()
                        cleantext = remove_double_newline_and_truncate(
                            BeautifulSoup(post_content, "lxml").text
                        )
                        ...
                        post = replace_tag(
                            post, "<!--|||||article_category|||||-->", category
                        )
                        post = replace_tag(
                            post,
                            "<!--|||||article_category_encoded|||||-->",
                        ...
                        post = replace_tag(
                            post, "<!--|||||article_post|||||-->", post_content
                        )
                        truncated_title = truncate(title)
                        url = replace_all_components(
                            quote(os.path.join("/", category, truncated_title) + "/")
                            .replace("//", "/")
                            .lower()
                        )
                        post = replace_tag(post, "<!--|||||link_url|||||-->", url)
                        post = replace_tag(
                            post, "<!--|||||full_url|||||-->", base_url + url
                        )
                        post = replace_all_components(post)
                        hash_value.update(post.encode())
                        if (
                            url not in content_database
                            or content_database[url]["hash_value"]
                            != hash_value.hexdigest()
                        ):
                            make_directory(truncated_title)
                            with open(
                                os.path.join("./", truncated_title, "index.html"),
                                mode="w",
                                encoding="utf-8",
                            ) as file:
                                file.write(post)
                        ...
                        category_database[category].append(
                            {"url": url, "title": title, "description": cleantext}
                        )
            os.chdir("..")
    os.chdir(original_directory)
...
def generate_pages(directory: str, website_directory: str):
    ...
    original_directory = os.getcwd()
    with open(
        os.path.join(original_directory, directory, "alias.csv"),
        mode="r",
        encoding="utf-8",
    ) as csv_file:
        csv_reader = csv.reader(csv_file)
    ...
    with open(
        os.path.join(original_directory, directory, "page_components.csv"),
        mode="r",
        encoding="utf-8",
    ) as csv_file:
        csv_reader = csv.reader(csv_file)
        ...
        for row in csv_reader:
            if -1 in (father_index, tag_index, child_index):
                ...
                with open(
                    os.path.join(original_directory, directory, row[child_index]),
                    mode="r",
                    encoding="utf-8",
                ) as file:
    ...
    for page in pages:
        if page.endswith(".html"):
            with open(
                os.path.join(original_directory, directory, page),
                mode="r",
                encoding="utf-8",
            ) as file:
                post_content = file.read()
                ...
                url = replace_all_components(
                    quote(os.path.join("/", truncated_title) + "/")
                    .replace("//", "/")
                    .lower()
                )
                post = replace_tag(post, "<!--|||||link_url|||||-->", url)
...
def generate_categories(website_directory: str):
    ...
    for category in category_database:
        ...
        post = replace_tag(
            post,
            "<!--|||||description|||||-->",
            "「" + category + "」系列所有文章列表 - 大陸居民臺灣正體字講義",
        )
        url = replace_all_components(
            quote(os.path.join("/category/", category) + "/").replace("//", "/").lower()
        )
        post = replace_tag(post, "<!--|||||link_url|||||-->", url)
        post = replace_tag(post, "<!--|||||full_url|||||-->", base_url + url)
        ...
        if (
            ...
            make_directory(category)
            with open(
                os.path.join("./", category, "index.html"), mode="w", encoding="utf-8"
            ) as file:
                file.write(post)
            content_database[url] = {
                "type": "category",
...
def generate_search(website_directory: str):
    ...
    post = replace_all_components(post)
    post = replace_tag(
        post, "<!--|||||description|||||-->", "搜索 - 大陸居民臺灣正體字講義"
    )
    url = "/search/"
    post = replace_tag(post, "<!--|||||link_url|||||-->", url)
    post = replace_tag(post, "<!--|||||full_url|||||-->", base_url + url)
...
def main(
    website_directory: str = "./NTCCTMCR/",
    entries_filepaths: str = ["../import.csv", "../Dictionaries/import_one_to_one.csv"],
    entries_category_names: str = ["一簡多繁辨析", "一簡一繁對應"],
    ...
    for i in range(len(entries_filepaths)):
        generate_entries(
            entries_filepaths[i], entries_category_names[i], website_directory
        )
